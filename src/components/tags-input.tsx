    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2 rounded-lg border border-border-primary bg-surface p-2 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
        {value.map((tag, index) => (
          <span
            key={tag}
            className="inline-flex items-center gap-1 rounded-full bg-emerald-900/30 px-3 py-1 text-sm font-medium text-emerald-400"
          >
            #{tag}
            <button
              type="button"
              onClick={() => removeTag(index)}
              className="ml-1 cursor-pointer hover:text-emerald-200"
            >
